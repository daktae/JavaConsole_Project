...
## 문제해결을 위한 노력
- 웹을 배우지 않은 단계라서, 메인화면을 자바 콘솔로 한땀한땀 구현해야했던 것이 문제였지만, 이미지를 아스키 코드를 바꿔주는 아스키 이미지 덕분에 메인화면을 쉽게 구성할 수 있었다.

## :memo: 아쉬운 점
- 웹을 배우지 않은 단계라서, 모든 ui를 자바 콘솔로 구현해야했었지만, 나중에 웹을 배우게 된다면 홈페이지에 css와 html을 이용하여 ui를 구성해야겠다.
