...
public class QuizSave {
	...
	/**
	 *  관리자가 입력한 퀴즈의 문제와 정답을 QuizDAO로부터 가져와 quizList.txt 파일에 저장하는 메소드입니다.
	 */
	public static void saveQuiz() {
		
			try {
				
				BufferedWriter writer = new BufferedWriter(new FileWriter(QUIZPATH));
				
				writer.write(String.format("%s■%s", QuizDAO.getQuizList().get(0).getQuestion()
												  , QuizDAO.getQuizList().get(0).getAnswer())); // 덮어쓰기
